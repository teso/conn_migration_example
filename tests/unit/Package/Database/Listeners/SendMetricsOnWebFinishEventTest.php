...
use Package\Database\Listeners\SendMetricsOnWebFinishEvent;
use Package\MetricsService\Interfaces\StatsdInterface;
use Package\MetricsService\Metrics\Database;
use Package\MetricsService\MetricsInfo;
use Package\MetricsService\StatsdFactory;
use Package\Web\Events\FinishEvent;
use TApplication;

class SendMetricsOnWebFinishEventTest extends Unit
{
    ...
    protected function _before()
    {
        ...
        $metricsInfoStub = Stub::make(
            MetricsInfo::class,
            ['getModule' => fn () => self::MODULE, 'getAction' => fn () => self::ACTION]
        );
        $applicationStub = Stub::make(TApplication::class, ['metricsInfo' => $metricsInfoStub]);

        Test::double(StatsdFactory::class, ['createStatsd' => fn () => $statsdMock]);
        Test::double(TApplication::class, ['getInstance' => fn () => $applicationStub]);
    ...
    public function testHandleWhenCountersAreEqualZero()
    {
        ...
        $listener = new SendMetricsOnWebFinishEvent();
